...
<form action="?page=simplr_reg_set&regview=main" method="post" id="add-field">
	<h3><?php _e('Core Settings','simplr-registration-form'); ?></h3>
	<?php
	SREG_Form::related_select(array(
		'name'=>'register_redirect',
		'label'=>__('Redirect default signup action to this page','simplr-registration-form'),
		'required'=>false,
		'post_type'=>'page',
		'comment'=>__('Use this option to redirect the default WordPress wp-login.php?action=redirect action. This will not redirect your login page as a whole.','simplr-registration-form')),
	...
	SREG_Form::related_select(array(
		...
		'label'=>__('Redirect default profile page','simplr-registration-form'),
		'required'=>false,
		'post_type'=>'page',
		'comment'=>__('Use this option to force users to access only their "front" end profile page ... i.e. the page with the [profile_page] shortcode','simplr-registration-form')),
	...
	SREG_Form::related_select(array(
		'name'			=>'thank_you',
		'label'			=>__('Custom Thank You Page','simplr-registration-form'),
		'required'	=>false,
		'post_type'	=>'page',
		'comment' 	=> __('You can leave this field blank. The default behavior outputs a confirmation on the page with the registration shortcode after registering. Use this only if you want to redirect the user elsewhere after registration.','simplr-registration-form')),
	...
	SREG_Form::text(array(
		'name'=>'default_email',
		'label'=>__('Default FROM Email','simplr-registration-form'),
		'required'=>false
	),
	$default_email, 'wide'
	);
	SREG_Form::radio(array(
		'name'=>'styles',
		'label'=>__('Disable Plugin Styles','simplr-registration-form'),
		'required'=>'no',
		'default' => 'no'
		), @$simplr_reg->styles, '',
	...
	if( isset($simplr_reg->styles) && $simplr_reg->styles != 'yes' ):
		SREG_Form::select(array(
			'name'=>'style_skin',
			'label'	=> __('Select a style','simplr-registration-form'),
			'required'	=> 'no',
			'default'	=> 'default.css',
			'helper'	=> 'skins'),
		...
		SREG_Form::text(array(
			'name'=>'stylesheet',
			'label'=>__('Override Default Stylesheet','simplr-registration-form'),
			'required'=>false,
			'comment'=>__('Specify a custom stylesheet. Will not apply if "Plugin Styles" are disabled','simplr-registration-form')
		),
	...
	<p class="submit">
		<input type="submit" class="button-primary" name="main-submit" value="<?php _e('Save Changes','simplr-registration-form') ?>" />
	</p>
</form>
